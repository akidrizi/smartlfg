...
if [ "$newest" -gt "$claude_mtime" ]; then
    printf '%s\n' '{"decision":"block","reason":"CLAUDE.md is older than changed source files (src/*.lua or SmartLFG.toc). Reconcile CLAUDE.md with the code changes (audit load order, file responsibilities, DB schema, commands, events, conventions and limitations — see the \"Keeping this file in sync\" section in CLAUDE.md), then re-save CLAUDE.md (even if already aligned) so it is at least as new as the sources."}'
fi
exit 0
